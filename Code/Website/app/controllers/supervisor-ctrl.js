...
***********************/
 ...
 $scope.changeDisplayMode = function changeDisplayMode() {
  ...
  $scope.parseCSV = function(){
    ...
    Papa.parse(file, {
      complete: function(results) {
        $scope.$apply(function () { $scope.parsedWatchOrders = results.data; });
     ...
     ***********************/
      $scope.getWatchOrders = function getWatchOrders(){
          dataService.viewWatchOrders()
            .then(
            function (data) {
              var watch_orders = []; //initialize an empty array to store results from the database
              for (var x in data) {               //for each category in the result
                //create an object and set object properties
                  var tmp = new Object();
                  tmp.Id = data[x].Id;
                  tmp.Desc = data[x].Desc;
              ...
              $scope.watch_orders = watch_orders;  //update for use in view
            },
            function (error) { console.log('Error: ' + error); });
      };
